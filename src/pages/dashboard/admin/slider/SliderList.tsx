...
import { useDeleteSliderMutation, useGetSliderAllListQuery } from "@/store/api/slider/sliderApi";
...
const SliderList: FC<ISliderListProps> = () => {
  ...
  const { data: slidersData, isLoading: sliderLoading } = useGetSliderAllListQuery({});

  const [deleteSlider] = useDeleteSliderMutation();

  useEffect(() => {
    const customizeSliderData = slidersData?.data?.map((singleSlider: slider, index: number) => ({
      ...singleSlider,
      index: index + 1,
    }));
      ...
      cell: ({ row }) => {
        ...
        return sliderItem ? (
          <div className="size-8 rounded-md overflow-hidden">
            <PhotoViewer className="scale-1" src={sliderItem.image} alt={`Slider Image ${sliderItem.id}`} />
          </div>
        ) : null;
      },
    },
            ...
            <DropdownMenuContent align="end" className="flex flex-col gap-1">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="w-full flex justify-start" size="xs">
                    Update
                  </Button>
                </DialogTrigger>
  ...
  return (
    <PageWrapper>
      <TableWrapper subHeading="Slider List and Image Management" heading="Sliders">
        <TableToolbar alignment="end">
          <ul className="flex items-center gap-x-2">
            
            <li>
              <Dialog open={sliderState.addSliderOpen} onOpenChange={(open) => setSliderState({ ...sliderState, addSliderOpen: open })}>
                <DialogTrigger asChild>
