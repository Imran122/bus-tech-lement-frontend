...
const UpdateCoachConfigNavigationForm: React.FC = () => {
  ...
  const [updateCoachConfiguration, { isLoading: isUpdating }] =
    useUpdateCoachConfigurationMutation();
  ...
  const handleSave = async (index: number) => {
    const coach = localData[index];
    const payload = {
      discount: coach.discount ? Number(coach.discount) : 0,
      registrationNo: coach.registrationNo,
      supervisorId: coach.supervisorId,
      driverId: coach.driverId,
      helperId: coach.helperId,
      tokenAvailable: coach.tokenAvailable ? Number(coach.tokenAvailable) : 0,
    };
    ...
    if (result?.data?.success) {
      ...
      toast({
        title: "Error",
        description: "Failed to update coach configuration.",
      });
    }
  };
  ...
  return (
    <FormWrapper
      heading="Update Coach Configuration"
      subHeading="Edit and save coach configuration details."
    >
      {/* Date Selector */}
      <div className="mb-4">
      ...
      <table className="min-w-full bg-background">
        <thead>
          <tr>
            <th className="border px-4 py-2">Coach Configuration</th>
            ...
            <th className="border px-4 py-2">Helper</th>
            <th className="border px-4 py-2">Token Available</th>
